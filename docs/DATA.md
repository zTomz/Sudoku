...
- Games, notes, undo history, time, preferences, completion results and completed learning lessons are stored locally through shared_preferences. The current validated snapshot and its previous validated recovery copy are retained so a damaged current value can fall back locally.
...
- Optional learning links open YouTube only after an explicit tap. The app embeds no YouTube player, loads no remote thumbnail and makes no YouTube request while a lesson is merely viewed. After opening the link, YouTube/Google processes the visit under its own terms; Sudoku does not append game or learning data.
