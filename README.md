...
| **Fresh boards, your pace** | Locally generated puzzles with a unique solution, in easy, medium and hard levels. |
| **Tools for solving** | Pencil notes, automatic note cleanup, undo, redo and erasing. Choose a cell first or select a number first. |
| **Just enough guidance** | Optional conflict or solution checking. Completed digits turn into checkmarks in the number bar. |
...
Difficulty currently uses clue-count targets rather than a human-solving technique rating. Explanatory hints, backup/export and additional Sudoku variants are not implemented yet; improvements in these areas are welcome topics for discussion.
